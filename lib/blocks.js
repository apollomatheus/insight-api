...
/**
 * This endpoint fetch the latest blocks
 * @limit number of blocks to return
 * @param req
 * @param res
 */
BlockController.prototype.getLatestBlocks = function(req, res) {
  var self = this;

  var latestBlocks = self.latestBlocksCache.get('blocks');
  // If we got nothing in cache or we have new blocks let's get the latest 5
  // TODO: Fetch only the difference
  if(!latestBlocks || latestBlocks.blocks[0].height < this.node.services.bitcoind.height) {
    self._list(null, req.query.limit || LATEST_BLOCK_LIMIT, function(data) {
      self.latestBlocksCache.set('blocks', data);
      res.jsonp(data);
    });
  } else {
    res.jsonp(latestBlocks);
  }
};
