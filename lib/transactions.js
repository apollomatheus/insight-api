 ...
 * @param res
 */
TxController.prototype.getLatestTransaction = function(req, res) {
  var self = this;
  var newTxCallback = function(data) {
    // kill that timer for the response timing out.
    if(timer) {
      clearTimeout(timer);
    }

    self.common.log.info('Insight-api: responding tx');
    res.jsonp(data)
  };

  messageBus.once('newTx', newTxCallback);

  var timer = setTimeout(function() {
    // too long has passed so remove listener and end response.
    messageBus.removeListener('newTx', newTxCallback);
    res.status(408).send();
  }, 60 * 5 * 1000);
};

TxController.prototype.emitNewTx = function(tx) {
  this.common.log.info('Insight-api: emitNewTx', tx);
  messageBus.emit('newTx', tx);
};
