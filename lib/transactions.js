...
function TxController(node) {
  ...
  var providers = require('./keys/testnet/providers.json');
  if(global.network === 'livenet' || global.network === 'mainnet') {
    providers = require('./keys/mainnet/providers.json');
  }
...
TxController.prototype.getASPKeyInfo = function(transaction) {
  var self = this;
  var getKey  = function(vout) {
    var scriptAsm = vout.scriptAsm.split(' ')[1];
    // Remove first hex byte
    var key = scriptAsm.slice(2);
    // Remove key id which is the last 4 hex bytes
    key = key.substring(0, key.length - 8);
    // Key id is the last 4 hex bytes
    var id = key.slice(key.length - 8);
    return self.providerStrings[key] || { pubkey: key, keyid: id };
  };

  return transaction.outputs.slice(1).map(getKey);
};

// TODO: Return important information about this
TxController.prototype.getProvisioningTxInfo = function(transaction) {
  var self = this;
  var type = self.getAdminKeyType(transaction);
  // asp keys, there is also a final 4 bytes which is the key id
  if (type === self.common.adminTypes.ASP_KEY_ADD || type === self.common.adminTypes.ASP_KEY_REVOKE) {
    return {
      type: type,
      keys: self.getASPKeyInfo(transaction)
    };
  }

  return  {
    type: type || this.common.adminTypes.PROVISIONING_TX,
    keys: self.getAddOrRemoveKeys(transaction)
  };

};

TxController.prototype.getAddOrRemoveKeys = function(transaction) {
  var self = this;
  var getKey  = function(vout) {
    var scriptAsm = vout.scriptAsm.split(' ')[1];
    var key = scriptAsm.slice(2);

    return self.providerStrings[key] || { pubkey: key };
  };

  return transaction.outputs.slice(1).map(getKey);
};

TxController.prototype.getAdminKeyType = function(transaction) {
  var scriptAsm = transaction.outputs[1].scriptAsm.split(' ')[1];
  var firstHexByte = scriptAsm.substring(0,2);

  var firstAdminASMHexByte = this.common.firstAdminASMHexBytes();
  return firstAdminASMHexByte[firstHexByte];
};
...
TxController.prototype.getRootThreadInfo = function(transaction) {
  ...
  var type = self.getAdminKeyType(transaction);
  return  {
    type: type || this.common.adminTypes.ROOT_THREAD,
    keys: self.getAddOrRemoveKeys(transaction)
  };
};
...
/**
 * This endpoint returns the latest transactions from the node
 * It transform the result to be frontend compatible
 * @param req
 * @param res
 */
TxController.prototype.getLatestTransactions = function(req, res) {
  var self = this;
  var limit =  req.query.limit || MAX_TX_POOL;

  var latestTxs = this.node.services.bitcoind.latestTransactionsPool;

  if(latestTxs.length > limit) {
    latestTxs = latestTxs.slice(0, limit);
  }

  if(latestTxs.length) {
    latestTxs = latestTxs.map(this.transformInvTransaction);
  }
